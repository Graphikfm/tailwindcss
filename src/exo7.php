...
if ($year === null) {
    ...
    header('Location: /src/exo7.php?year='.$year);
}
...
if (($year > $dateFixe+5) || ($year < $dateFixe-5)) {
    header('Location: /src/exo7.php?year='.$dateFixe);
}
$daysName = [
    0=>'Dimanche',
    1=>'Lundi',
...
?>
<div class="global_container rounded-xl w-1/3 h-3/5 bg-gray-200 flex m-7 justify-center relative shadow-md  p-7">
    <?php
    if (($year == $date)|| !$year){
        $year = (int) (new DateTime())->format('Y');
//     var_dump($year);
...
//    ($year > $dateFixe+5)
    if ($year === ($dateFixe-5)) {
        ?>
        <div class="container_btn_left text-white justify-center items-center not-allowed shadow-black shadow-md bg-red-200 w-16 h-16 rounded-lg absolute top-0 left-0 flex">
<!--            <div class=" not-allowed  top-0 left-0 flex justify-center items-center w-16 h-16 text-white" >-->
...
<!--            </div>-->
    ...
    <?php } else { ?>
        <a class=" container_btn_left shadow-black shadow-md bg-gray-900 w-16 h-16 rounded-lg absolute top-0 left-0 flex justify-center items-center text-white" href="/src/exo7.php?year=<?php echo $sub ?>">
            <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-arrow-left" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                <path d="M5 12l14 0"></path>
                <path d="M5 12l6 6"></path>
    ...
    if ($year === ($dateFixe+5)) {
        ?>
        <div class="container_btn_left not-allowed shadow-black shadow-md bg-red-200 w-16 h-16 rounded-lg flex absolute top-0 right-0 justify-center items-center text-white">
<!--            <a class=" not-allowed  top-0 left-0 flex justify-center items-center w-16 h-16 text-white">-->
...
<!--            </a>-->
    ...
    <?php } else {  ?>
    <a class=" container_btn_right shadow-black shadow-md bg-gray-900 w-16 h-16 rounded-lg flex absolute top-0 right-0 justify-center items-center text-white" href="/src/exo7.php?year=<?php echo $add ?>">
        <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-arrow-right" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
            <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
            <path d="M5 12l14 0"></path>
            <path d="M13 18l6 -6"></path>
